...
| [oidc-harness-wiring-checklist.md](casehub/oidc-harness-wiring-checklist.md) | Five mandatory steps when adding casehub-platform-oidc: exclude TenantScopedPrincipal, set application-type=service, add dev-profile bypass, add test OIDC config, add @TestSecurity to all HTTP-calling @QuarkusTest classes | All harnesses adopting casehub-platform-oidc (life, devtown, clinical, openclaw) |
| [rbac-classifier-context-guard.md](casehub/rbac-classifier-context-guard.md) | @ApplicationScoped ActionRiskClassifier injecting @RequestScoped CurrentPrincipal must catch ContextNotActiveException at each groups() call site; pass pre-computed admin flag to avoid redundant proxy invocations | Harness apps with role-differentiated ActionRiskClassifier (e.g. life LifeActionRiskClassifier) |
